...
public class Tree<V> implements Iterable<Tree<V>> {
    ...
    public Tree(V value, Tree<V>... children) {
    	...
    	this.value = value;
    	if (children != null) {
    		for (Tree<V> child: children) this.children.add(child);
    	}
    }
     ...
     */
    public V getValue() {
        return this.value; // TODO Replace with correct result
    }
    ...
    public void addChild(int index, Tree<V> child) {
        // TODO
    	if (index < 0 || index > this.children.size() - 1) throw new IndexOutOfBoundsException("Given index not valid");
    	if (this.contains(child)) throw new IllegalArgumentException("Given child already exists in the tree");
    ...
    public int getNumberOfChildren() {
        return this.children.size(); // TODO Replace with correct result
    }
    ...
    public Tree<V> getChild(int index) {
    	if (index < 0 || index > this.children.size() - 1) throw new IndexOutOfBoundsException("Given index not valid");
        return this.children.get(index); // TODO Replace with correct result
    }
    ...
    public Iterator<Tree<V>> iterator() {
        return this.children.iterator(); // TODO Replace with correct result
    }
    ...
    boolean contains(Tree<V> node) {
    	// TODO
    	Queue<Tree<V>> nodeQ = new LinkedList<Tree<V>>();
    	nodeQ.add(this);
    	while(!nodeQ.isEmpty()) {
    		Tree<V> temp = nodeQ.remove();
    		if (temp == node) return true;
    		for (Tree<V> subnode: temp.children) nodeQ.add(subnode);
    	}
    	return false;
    }
    ...
    public String toString() {
        ...
        // TODO Replace with correct result
    }
    
    public String toString(Tree<V> node) {
    	String result = "";
    	if (node == null) return "";
    	result += node.getValue();
    	if (node.getNumberOfChildren() > 0) {
    		result += " (";
    		for (Tree<V> child: node.children) result += toString(child) + " ";
